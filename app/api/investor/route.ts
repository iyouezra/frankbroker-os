...
export async function GET(request: Request) {
  try {
        ...
        include: {
          accounts: {
            include: {
              ...
              corporateActionEntitlements: { include: { corporateAction: { include: { instrument: true } }, taxCalculations: { orderBy: [{ calculatedAt: "desc" }, { revision: "desc" }], take: 1 } }, orderBy: { createdAt: "desc" }, take: 50 },
              taxLots: { include: { instrument: true, dispositions: { include: { saleTrade: true, corporateActionEntitlement: { include: { corporateAction: true } }, taxCalculations: { orderBy: [{ calculatedAt: "desc" }, { revision: "desc" }], take: 1 } } } }, orderBy: { createdAt: "desc" }, take: 100 },
            },
          },
    ...
    return Response.json({
      ...
      servicing: account ? {
        basisSummary: {
          totalLots: account.taxLots.length,
          unknownBasisLots: account.taxLots.filter((lot) => lot.basisStatus === "unknown").length,
          dispositionsNeedingBasis: account.taxLots.flatMap((lot) => lot.dispositions).filter((item) => item.realizedGain === null).length,
        },
        income: account.corporateActionEntitlements.map((item) => ({
          id: item.id,
          actionId: item.corporateActionId,
          instrument: item.corporateAction.instrument.symbol,
          type: item.corporateAction.actionType,
          recordDate: item.corporateAction.recordDate.toISOString().slice(0, 10),
          paymentDate: item.corporateAction.paymentDate.toISOString().slice(0, 10),
          grossCash: item.grossCash === null ? null : toNum(item.grossCash),
          withholdingAmount: item.withholdingAmount === null ? null : toNum(item.withholdingAmount),
        ...
        realizations: account.taxLots.flatMap((lot) => lot.dispositions.map((item) => ({
          id: item.id,
          tradeId: item.saleTradeId,
          tradeDate: (item.saleTrade?.tradeDate ?? item.corporateActionEntitlement!.corporateAction.paymentDate).toISOString().slice(0, 10),
          instrument: lot.instrument.symbol,
          quantity: toNum(item.quantity),
          netProceeds: toNum(item.netProceeds),
