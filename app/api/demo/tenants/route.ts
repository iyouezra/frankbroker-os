...
export async function GET() {
  try {
    ...
    return Response.json({ tenants: contexts.map((item) => ({ id: item.id, tradingName: item.tradingName, businessType: item.businessType, modules: item.modules, availableRoles: item.availableRoles })) });
