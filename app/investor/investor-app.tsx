...
function InvestmentOnboarding({ onDone }: { onDone: () => void }) {
  ...
  if (step === 0) return <div className={styles.onboarding}><button className={styles.skip} onClick={onDone}>Skip for now</button><div className={styles.onboardingIntro}><Image src="/frankscore-icon.png" alt="Frank" width={74} height={104} priority /><h1>Let&apos;s find<br />your strategy</h1><p>Three honest questions. No jargon. We&apos;ll suggest a mix of ESX stocks and government bonds that may fit you.</p></div><Button className={styles.full} onClick={() => setStep(1)}>Start</Button><ProgressDots step={0} /></div>;
