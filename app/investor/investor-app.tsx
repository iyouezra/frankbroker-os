...
function HomeScreen({ openStock, go, account, unread, onBell }: { openStock: (stock: InvestorStock) => void; go: (tab: Tab) => void; account: InvestorBootstrap["account"]; unread: number; onBell: () => void }) {
...
export default function InvestorApp() {
  ...
  const postInvestor = async (body: unknown) => {
    ...
    const data = await response.json().catch(() => ({})) as { error?: string; order?: { id: string; status: string }; checks?: OrderCheck[]; request?: { id: string; status: string } };
