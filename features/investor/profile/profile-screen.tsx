...
export function ProfileScreen({ notify, name, profile, accountNumber, orders, requests, legalDocument, documents, linkedBanks, supportUnread, passkeysEnabled, onOpenSupport, onOpenRequest, onAddBank, onDeleteBank, onAcceptTerms, onUpdateKyc, onRequest, onDownloadStatement, onSignOut }: { notify: (message: string) => void; name: string; profile: InvestorBootstrap["profile"]; accountNumber?: string | null; orders: NonNullable<InvestorBootstrap["account"]>["orders"]; requests: InvestorBootstrap["serviceRequests"]; legalDocument: InvestorBootstrap["tenant"]["legalDocument"]; documents: InvestorBootstrap["documents"]; linkedBanks: LinkedBankAccount[]; supportUnread: number; passkeysEnabled: boolean; onOpenSupport: () => void; onOpenRequest: (threadId: string) => void; onAddBank: (bankName: string, accountNumber: string) => Promise<void>; onDeleteBank: (id: string) => Promise<void>; onAcceptTerms: () => Promise<void>; onUpdateKyc: (files: Partial<Record<string, File>>) => Promise<void>; onRequest: (input: ServiceRequestInput) => Promise<boolean>; onDownloadStatement: (from: string, to: string) => Promise<boolean>; onSignOut?: () => Promise<void> }) {
  const t = useT();
  ...
  return <div className={styles.screen}>
    ...
    {workflow === "security" && <SecuritySheet profile={profile} passkeysEnabled={passkeysEnabled} onClose={() => setWorkflow(null)} onSubmit={onRequest} />}
