...
type CommissionPromotion = { name: string; eligibility: "all_clients" | "new_clients"; startsOn: string; endsOn: string; newClientWindowDays: number | null };
...
type SettingsControls = { brokerageFeePct: number; minimumFee: number; approvalThreshold: number; clientDailyLimit: number; makerChecker: boolean; allowedOrderTypes: string[]; settlementCycle: string; feeRules: SettingsFeeRule[]; feeScheduleVersion: string; feeScheduleEffectiveFrom: string | null; regulatoryFeeScheduleVersion: string; marketFeeScheduleConfigured: boolean; commissionPromotion: CommissionPromotion | null };
...
export function SettingsPage() {
  ...
  const togglePromotion = () => {
    ...
    const startsOn = controls.feeScheduleEffectiveFrom && controls.feeScheduleEffectiveFrom > today ? controls.feeScheduleEffectiveFrom : today;
    const endsOn = new Date(`${startsOn}T00:00:00.000Z`).getTime() + 30 * 86_400_000;
    updateControl({ commissionPromotion: controls.commissionPromotion ? null : { name: "Commission-free offer", eligibility: "new_clients", startsOn, endsOn: new Date(endsOn).toISOString().slice(0, 10), newClientWindowDays: 30 } });
  };
  const updatePromotion = (patch: Partial<CommissionPromotion>) => {
    if (controls?.commissionPromotion) updateControl({ commissionPromotion: { ...controls.commissionPromotion, ...patch } });
  };
  const publishBrokerageFees = async () => {
    if (!controls) return;
    setSavingFees(true); setFeeMessage("");
    try {
      const response = await fetch("/api/tenant", { method: "PATCH", headers: { "content-type": "application/json", "x-frank-tenant-id": BROKER_TENANT_ID, "x-frank-demo-role": "broker_admin" }, body: JSON.stringify({ action: "brokerage_fee_schedule", version: controls.feeScheduleVersion, effectiveFrom: controls.feeScheduleEffectiveFrom, rules: controls.feeRules.map(({ assetClass, marketSegment, brokeragePct, minimumFee, maximumFee, tiers }) => ({ assetClass, marketSegment, brokeragePct, minimumFee, maximumFee, tiers: tiers ?? [] })), promotion: controls.commissionPromotion }) });
  ...
  return <>
    ...
    {controls && <section className="panel brokerage-settings"><div className="panel-head"><div><span className="eyebrow">TENANT-MANAGED · VERSIONED</span><h2>Brokerage commission</h2><p>Set your own commission and minimum by instrument class. Market and regulatory rates are shown for context and cannot be edited here.</p></div><button className="btn primary" disabled={savingFees} onClick={() => void publishBrokerageFees()}>{savingFees ? "Publishing…" : "Publish commission schedule"}</button></div>
      ...
      <section className="settings-note commission-promotion"><div><b>Commission-free promotion</b><p>Waive the broker&apos;s commission during a controlled offer. Regulatory, exchange, and CSD charges still apply.</p></div><button className={`btn ${controls.commissionPromotion ? "danger" : "secondary"} small`} onClick={togglePromotion}>{controls.commissionPromotion ? "Remove promotion" : "＋ Add promotion"}</button>{controls.commissionPromotion && <div className="brokerage-version-fields"><label>Offer name<input value={controls.commissionPromotion.name} onChange={(event) => updatePromotion({ name: event.target.value })} /></label><label>Eligible clients<BrandSelect value={controls.commissionPromotion.eligibility} onChange={(eligibility) => updatePromotion({ eligibility: eligibility as CommissionPromotion["eligibility"], newClientWindowDays: eligibility === "new_clients" ? controls.commissionPromotion?.newClientWindowDays ?? 30 : null })} ariaLabel="Promotion eligibility" options={[{ value: "new_clients", label: "New clients" }, { value: "all_clients", label: "All clients" }]} /></label>{controls.commissionPromotion.eligibility === "new_clients" && <label>Free-trading window (days)<input type="number" min="1" max="365" value={controls.commissionPromotion.newClientWindowDays ?? 30} onChange={(event) => updatePromotion({ newClientWindowDays: Number(event.target.value) })} /><small>Counted from client approval.</small></label>}<label>Starts on<input type="date" value={controls.commissionPromotion.startsOn} onChange={(event) => updatePromotion({ startsOn: event.target.value })} /></label><label>Ends on<input type="date" value={controls.commissionPromotion.endsOn} onChange={(event) => updatePromotion({ endsOn: event.target.value })} /></label></div>}</section>
