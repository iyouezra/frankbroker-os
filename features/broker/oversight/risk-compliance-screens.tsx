...
type EmployeeData = {
  ...
  dealingRegister: Array<{ id: string; side: string; quantity: string; price: string; status: string; submittedAt: string | null; employeeProfile?: { user: { fullName: string } }; instrument: { symbol: string; name: string }; trades: Array<{ id: string; quantityFilled: string; executionPrice: string }>; personalTradeClearance?: { id: string; status: string } | null; monitoringAlerts: Array<{ id: string; ruleCode: string; severity: string; status: string }> }>;
};
...
export function RiskComplianceWorkspace({ mode, role, tenantId, onNotify }: { mode: Mode; role: Role; tenantId: string; onNotify: (message: string, tone?: "success" | "error") => void }) {
  ...
  const [employees, setEmployees] = useState<EmployeeData>({ profiles: [], clearances: [], restrictions: [], disclosures: [], attestations: [], sensitiveAccess: [], users: [], instruments: [], dealingRegister: [] });
  ...
  const runSweep = async () => {
    ...
    onNotify(`Monitoring completed · ${result.clientAlerts ?? 0} client and ${result.employeeAlerts ?? 0} employee-control alert records evaluated.`);
    await load();
  };
  ...
  if (mode === "employee") return <>
    <SectionHeader eyebrow="INTERNAL CONDUCT" title="Employee conduct" copy="In-house employee accounts, personal-trade clearances, restricted securities, disclosures and annual attestations." action={<button className="primary-button" onClick={() => void runSweep()}>Run control sweep</button>} />
    ...
    <section className="panel table-panel"><div className="panel-head"><div><span className="eyebrow">AUTOMATIC REGISTER</span><h2>In-house personal dealing</h2></div><p>Derived from employee-linked orders and executions.</p></div><div className="table-scroll"><table><thead><tr><th>Employee</th><th>Date</th><th>Security</th><th>Order</th><th>Execution</th><th>Clearance / alerts</th></tr></thead><tbody>{employees.dealingRegister.map((item) => <tr key={item.id}><td>{item.employeeProfile?.user.fullName ?? "Employee"}</td><td>{item.submittedAt ? new Date(item.submittedAt).toLocaleString() : "—"}</td><td><b>{item.instrument.symbol}</b><small>{item.instrument.name}</small></td><td>{displayLabel(item.side)} · {Number(item.quantity).toLocaleString()} @ {Number(item.price).toLocaleString()}<small>{item.id} · {displayLabel(item.status)}</small></td><td>{item.trades.length ? item.trades.map((trade) => <small key={trade.id}>{Number(trade.quantityFilled).toLocaleString()} @ {Number(trade.executionPrice).toLocaleString()}</small>) : "Not executed"}</td><td>{item.personalTradeClearance ? badge(item.personalTradeClearance.status) : "No linked clearance"}{item.monitoringAlerts.map((alert) => <small key={alert.id}>{alert.ruleCode} · {displayLabel(alert.status)}</small>)}</td></tr>)}</tbody></table></div>{!employees.dealingRegister.length && <EmptyState title="No employee dealing" copy="Orders on linked in-house employee accounts will appear automatically." />}</section>
