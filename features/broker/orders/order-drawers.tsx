...
export function NewOrderForm({ value, setValue, clients, instruments, controls, checks, busy, onInstructionChange, onValidate, onSubmit }: { value: NewOrderValue; setValue: (value: NewOrderValue) => void; clients: BrokerClient[]; instruments: BrokerInstrument[]; controls: TenantControls; checks: { label: string; passed: boolean; message: string }[] | null; busy: boolean; onInstructionChange: () => void; onValidate: () => void; onSubmit: (event: FormEvent) => void }) {
  ...
  return <form onSubmit={onSubmit} className="drawer-content">
    ...
    <div className="form-section"><h3>Client instruction</h3>
      ...
      <div className="field-row"><label>Order type<BrandSelect value={value.orderType} disabled={!controls.allowedOrderTypes.length} ariaLabel="Order type" onChange={(next) => updateInstruction({ orderType: next })} options={controls.allowedOrderTypes.length ? controls.allowedOrderTypes.map((orderType) => ({ value: orderType, label: orderType })) : [{ value: "", label: "No order types enabled" }]} /></label><label>Validity<BrandSelect value={value.validity} onChange={(next) => setValue({ ...value, validity: next })} ariaLabel="Validity" options={[{ value: "Day", label: "Day" }, { value: "Good till date", label: "Good till date" }, { value: "Immediate or cancel", label: "Immediate or cancel" }]} /></label></div>
