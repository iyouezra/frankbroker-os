...
function TradingMandatePanel({ clientId, mandate, canAdjust, role, onSaved }: { clientId: string; mandate: NonNullable<Client360Detail["tradingMandate"]> | undefined; canAdjust: boolean; role: Role; onSaved: () => void }) {
  const initial = mandate ?? { status: "active", buyEnabled: true, sellEnabled: true, maxOrderValue: null, dailyGrossLimit: null, effectiveDailyGrossLimit: 0, allowedAssetClasses: [], allowedMarketSegments: [], allowedOrderTypes: [], commissionSource: "tenant_default" as const, commissionRatePct: null, commissionMinimumFee: null, commissionMaximumFee: null, commissionEffectiveFrom: null, commissionReason: null, version: 1 };
  ...
  return <section className="panel"><div className="panel-head"><div><span className="eyebrow">CLIENT TRADING CONTROL</span><h2>Mandate and limits</h2><p>Blank client limits inherit the tenant policy. Broker administrators can record a client-specific commission with an effective date and audit reason.</p></div>{canAdjust && <button className="btn primary small" disabled={busy} onClick={() => void save()}>{busy ? "Saving…" : "Save mandate"}</button>}</div><div className="form-grid"><label>Status<BrandSelect value={value.status} onChange={(status) => setValue((current) => ({ ...current, status }))} ariaLabel="Trading mandate status" options={[{ value: "active", label: "Active" }, { value: "suspended", label: "Suspended" }]} /></label><label>Maximum order value (ETB)<input type="number" min="0" disabled={!canAdjust} placeholder="No client-specific cap" value={value.maxOrderValue ?? ""} onChange={(event) => setValue((current) => ({ ...current, maxOrderValue: event.target.value === "" ? null : Number(event.target.value) }))} /></label><label>Daily gross limit (ETB)<input type="number" min="0" disabled={!canAdjust} placeholder={`Tenant default ${value.effectiveDailyGrossLimit}`} value={value.dailyGrossLimit ?? ""} onChange={(event) => setValue((current) => ({ ...current, dailyGrossLimit: event.target.value === "" ? null : Number(event.target.value) }))} /></label><label>Allowed market segments<input disabled={!canAdjust} placeholder="Blank means all; comma-separated" value={value.allowedMarketSegments.join(", ")} onChange={(event) => setValue((current) => ({ ...current, allowedMarketSegments: event.target.value.split(",").map((item) => item.trim().toLowerCase()).filter(Boolean) }))} /></label></div><div className="order-types"><span>Directions</span><label><input type="checkbox" disabled={!canAdjust} checked={value.buyEnabled} onChange={(event) => setValue((current) => ({ ...current, buyEnabled: event.target.checked }))} /><i />Buy</label><label><input type="checkbox" disabled={!canAdjust} checked={value.sellEnabled} onChange={(event) => setValue((current) => ({ ...current, sellEnabled: event.target.checked }))} /><i />Sell</label></div><div className="order-types"><span>Asset classes <small>(none means all)</small></span>{[["equity", "Equity"], ["bond", "Government bond"]].map(([id, label]) => <label key={id}><input type="checkbox" disabled={!canAdjust} checked={value.allowedAssetClasses.includes(id)} onChange={() => toggle("allowedAssetClasses", id)} /><i />{label}</label>)}</div><div className="order-types"><span>Order types <small>(none means tenant defaults)</small></span>{[["market", "Market"], ["limit", "Limit"], ["stop_loss", "Stop-loss"]].map(([id, label]) => <label key={id}><input type="checkbox" disabled={!canAdjust} checked={value.allowedOrderTypes.includes(id)} onChange={() => toggle("allowedOrderTypes", id)} /><i />{label}</label>)}</div><div className="form-section"><h3>Commission terms</h3><div className="form-grid"><label>Pricing source<BrandSelect value={value.commissionSource} onChange={(commissionSource) => setValue((current) => ({ ...current, commissionSource: commissionSource as typeof current.commissionSource }))} disabled={role !== "broker_admin"} ariaLabel="Client commission pricing source" options={[{ value: "tenant_default", label: "Tenant schedule and tiers" }, { value: "client_override", label: "Client-specific rate" }]} /></label>{value.commissionSource === "client_override" && <><label>Commission rate (%)<input type="number" min="0" max="100" step="0.0001" disabled={role !== "broker_admin"} value={value.commissionRatePct ?? ""} onChange={(event) => setValue((current) => ({ ...current, commissionRatePct: event.target.value === "" ? null : Number(event.target.value) }))} /></label><label>Effective from<input type="date" disabled={role !== "broker_admin"} value={value.commissionEffectiveFrom ?? ""} onChange={(event) => setValue((current) => ({ ...current, commissionEffectiveFrom: event.target.value || null }))} /></label><label>Minimum fee (ETB)<input type="number" min="0" disabled={role !== "broker_admin"} placeholder="Inherit tenant minimum" value={value.commissionMinimumFee ?? ""} onChange={(event) => setValue((current) => ({ ...current, commissionMinimumFee: event.target.value === "" ? null : Number(event.target.value) }))} /></label><label>Maximum fee (ETB)<input type="number" min="0" disabled={role !== "broker_admin"} placeholder="Inherit tenant maximum" value={value.commissionMaximumFee ?? ""} onChange={(event) => setValue((current) => ({ ...current, commissionMaximumFee: event.target.value === "" ? null : Number(event.target.value) }))} /></label><label>Reason<input disabled={role !== "broker_admin"} value={value.commissionReason ?? ""} onChange={(event) => setValue((current) => ({ ...current, commissionReason: event.target.value || null }))} placeholder="Agreement or approval reference" /></label></>}</div><div className="settings-note"><b>{value.commissionSource === "client_override" ? "Client-specific commission" : "Tenant commission schedule"}</b><span>{value.commissionSource === "client_override" ? "Once effective, this rate replaces the tenant value tiers. Active commission-free promotions still take precedence." : "Uses the active tenant schedule and its order-value tiers."}</span></div></div>{message && <p className="control-message">{message}</p>}</section>;
}
...
export function ClientsPage({ clients, selectedId, onSelect, orders, instruments, role, focus, onNewClient, onRefresh, onOpenOrder, onOpenConversation }: { clients: BrokerClient[]; selectedId: string; onSelect: (id: string) => void; orders: DemoOrder[]; instruments: BrokerInstrument[]; role: Role; focus: { status?: string; clientId?: string; tab?: "overview" | "documents" } | null; onNewClient: () => void; onRefresh: () => Promise<void>; onOpenOrder: (order: DemoOrder) => void; onOpenConversation?: (threadId: string) => void }) {
  ...
  return <>
    ...
    {tab === "overview" && <div className="client-360-grid">
      <section className="panel onboarding-record"><div className="panel-head"><h2>Investment profile</h2></div><dl>
        <div><dt>Source of funds</dt><dd>{model.client.sourceOfFunds || "Not recorded"}</dd></div>
        {!model.client.investmentProfile && <div><dt>Investment objective</dt><dd>{model.client.investmentObjective || "Not recorded"}</dd></div>}
        {investmentProfileRows(model.client.investmentProfile).map((row) => <div key={row.label}><dt>{en[row.label]}</dt><dd>{en[row.answer]}</dd></div>)}
      </dl>{(!model.client.investmentProfile || investmentProfileRows(model.client.investmentProfile).some((row) => row.answer === "suitability.notRecorded")) && <p>Incomplete — follow up with the client about unanswered items.</p>}</section>
