...
export function Dashboard({ userName, orders, auditEntries, queue, settlementCycle, manualTradeCapture, onViewOrders, onNewOrder, onSettle, onOpenWork, onOpenStatus }: { userName: string; orders: DemoOrder[]; auditEntries: AuditEntry[]; queue: WorkItem[]; settlementCycle: string; manualTradeCapture: boolean; onViewOrders: () => void; onNewOrder: () => void; onSettle: () => void; onOpenWork: (item: WorkItem) => void; onOpenStatus: (status: "all" | "review" | "executed" | "exceptions") => void }) {
  ...
  return <>
    ...
    <div className="dashboard-grid">
      <section className="panel queue-panel"><div className="panel-head"><div><span className="eyebrow">MY WORK</span><h2>Needs your attention</h2></div>{queue.length > 0 && <span className="queue-count">{queue.length} open</span>}</div>
