...
const breakdown = (order: DemoOrder) => order.executedFeeBreakdown ?? order.estimatedFeeBreakdown ?? { brokerage: order.estimatedFees ?? 0, regulator: 0, exchange: 0, csd: 0, total: order.estimatedFees ?? 0 };
...
export function buildReports(orders: DemoOrder[], clients: BrokerClient[], audit: AuditRow[]): Report[] {
  ...
  return [
    ...
    },
    {
      ...
      rows: executed.map((order) => { const fees = breakdown(order); const policy = order.estimatedFeeBreakdown?.policy; return [order.id, order.client, order.symbol, money(order.estimatedGross), money(fees.brokerage), money(fees.regulator), money(fees.exchange), money(fees.csd), money(fees.total), policy?.brokerageScheduleVersion ?? "legacy", policy?.regulatoryScheduleVersion ?? "legacy"]; }),
    },
    {
      id: "audit-log",
