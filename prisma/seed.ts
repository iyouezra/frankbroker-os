...
async function main() {
  ...
  await prisma.user.createMany({
    data: [
      ...
      { id: "usr_blue_admin", brokerId: "brk_blue_nile", employeeId: "BNC-0001", email: "samuel@addiscapital.example", fullName: "Samuel Kebede", jobTitle: "Access Administrator", department: "Operations", role: "access_admin", status: "active", mfaEnabled: true },
      ...
      { id: "usr_blue_tenant_admin", brokerId: "brk_blue_nile", employeeId: "BNC-0002", email: "tenant.admin@addiscapital.example", fullName: "Mimi Solomon", jobTitle: "Tenant Administrator", department: "Operations", role: "broker_admin", status: "active", mfaEnabled: true },
  ...
  const tenantSettings = [
    ...
    { id: "set_brk_blue_nile", brokerId: "brk_blue_nile", tradingName: "Addis Capital", plan: "Growth", domain: "invest.addiscapital.example", supportEmail: "care@addiscapital.example", primaryColor: "#2277C8", welcomeMessage: "A simpler way to own ESX companies and government bonds.", businessDate: dateOnly("2026-07-14"), features: { investorPortal: true, selfDirected: true, bonds: true, recurringInvestments: false, institutionalAccounts: true, manualTradeCapture: true }, makerChecker: true, approvalThreshold: 100_000, clientDailyLimit: 750_000, brokerageFeePct: .65, minimumFee: 30, settlementCycle: "T+2", allowedOrderTypes: ["Market", "Limit"], requireTermsAcceptance: true, discrepancyWindowDays: 10, kycReviewMonths: 12 },
  ...
  await prisma.advisoryDeal.createMany({ data: [
    ...
    { id: "deal_sheba_highland_ipo", tenantId: "brk_sheba", issuerId: "iss_sheba_highland", checklistTemplateId: "tpl_ipo_growth_v1", name: "Highland Manufacturing Growth Market", transactionType: "ipo", marketSegment: "growth", mandateReference: "SHA-2026-003", leadUserId: "usr_sheba_tenant_admin", stage: "readiness", status: "active", targetDate: dateOnly("2027-02-15"), createdByUserId: "usr_sheba_tenant_admin" },
  ], skipDuplicates: true });
